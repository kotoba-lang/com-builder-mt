(ns builder-mt.main
  "Kotodama WASM entrypoint for the Builder_mt clean-room actor (L4) — Clojure port.
  ...
  facts (`builder_mt.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "builder_mt")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"      :plural "projects"      :id-prefix "builderm_pro"
    :fields [:jurisdiction :status]                :required [:jurisdiction]
    :coerce {}                                      :refs {}}
   {:entity "Permit"       :plural "permits"       :id-prefix "builderm_per"
    :fields [:projectId :type :number :status]     :required [:type :number]
    :coerce {}                                      :refs {:projectId "Project"}}
   {:entity "Drawing"      :plural "drawings"      :id-prefix "builderm_dra"
    :fields [:projectId :name :revision :contentRef] :required [:name :revision]
    :coerce {}                                      :refs {:projectId "Project"}}
   {:entity "Inspection"   :plural "inspections"   :id-prefix "builderm_ins"
    :fields [:permitId :type :result :inspectedAt] :required [:type :result]
    :coerce {}                                      :refs {:permitId "Permit"}}
   {:entity "Asset"        :plural "assets"        :id-prefix "builderm_ass"
    :fields [:type :location :condition]           :required [:type]
    :coerce {}                                      :refs {}}
   {:entity "ServiceRequest" :plural "servicerequests" :id-prefix "builderm_ser"
...
(defn emit-facts
  "EAVT facts for one record: {\"builder_mt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "builder_mt-compat" :tier tier :entities entities} 200])
...
(defn start! [] :builder_mt-compat/ready)
